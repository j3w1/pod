...
Guarded reservation checks each frozen packet source and source/instruction context path under the objective admission lock. A current absent source, even one frozen as absent, or a changed source durably rejects that packet assignment; an unavailable read holds admission without converting it to a definitive rejection. This is a bounded admission check, not an atomic multi-file snapshot against external writes. Reserved or uncertain release cleanup occupies objective and overlapping account capacity until exact native resource readback settles it; fixture tests cover the bookkeeping without proving a live native release.
