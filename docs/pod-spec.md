...
Non-assurance proof uses `pod-evidence/v1`, associated with the obligation and
bound to its candidate, sources, effective Governor policy, dependencies and
environment. Review admissions bind the same context; REUSE preserves only
unaffected bindings. A failed, incomplete or scope-unreconciled review observation
cannot satisfy assurance. A completed review may still require code corrections.
...
| A129 | Supported effort is used or omitted as native default; context is omitted without native control and effective metadata remains honest. |
...
| A147 | A review requirement cited from `AGENTS.md` at the base revision lands as a `project_policy` obligation. The same text from the issue body, another repository file, fetched documentation or an edit inside the candidate stays a proposal. `project_policy` cannot withdraw an objective criterion. At a changed target-branch base, unchanged or moved exact cited text remains authoritative; genuinely gone text permits justified withdrawal of that policy obligation only. Unavailable declared sources hold new work. |
...
| A153 | An overlapping implementation packet is refused `integration_pending` while a settled result has no disposition; a non-overlapping packet admits. `integrated_into` is validated by ancestry. A result that exceeded its boundary is recorded as `boundary_exceeded` and needs a reason before integration. The owner obligation cannot be satisfied while the disposition is missing.        |
...
| A155 | **Amplification.** A review packet must serve an assurance obligation. A second assurance obligation on an overlapping scope and the same candidate without `uncovered_risk` is refused `obligation_invalid`. A review attempt against a satisfied assurance obligation is refused `unbound_assignment`.                                                                            |
| A156 | A `required_correction` creates a parented correction obligation, and an `advisory` finding becomes a proposal. Downgrading a reviewer's blocker records a reason that the report lists. A correction affecting reviewed scope, assumptions, dependencies or governance, or with unknown impact, re-opens the same assurance obligation for delta review. Unaffected evidence receives an explicit candidate REUSE binding with the Git delta; stricter project policy governs. The coordinator cannot withdraw a correction.            |
| A157 | With an unbound assurance obligation, none at all, or only withdrawn assurances, the report stands, the label is withheld, and `assurance_unbound` names the gap. A valid REUSE binding satisfies the requirement.                                                                                                                                                                                         |
