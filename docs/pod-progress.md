...
| P03 | PARTIAL | Admission derives hard capacity and scoped exceptional grants from current effective policy; per-window quota holds preserve the latest exhaustion and require later fresh positive evidence. Global serialized admission counts overlapping pending launch and unresolved cleanup intentions across objectives, deduplicated against exact native Dispatch rows in synthetic fixtures. Reserved and uncertain release intentions retain capacity until exact readback reconciles them. Billing/reset guards, strict read adapter and one-shot effect reconciliation pass synthetic tests. Actual native billing/fan-out preflight and account-bucket binding remain unverified; production dispatch blocks. |
| P04 | PARTIAL | Typed bounded packet references, retained no-follow single-file source reads, and guarded packet-source and source/instruction-context checks pass synthetic fixtures at reservation. Proven changed or absent sources durably reject the packet assignment; unavailable observations hold without a definitive rejection. Native-bound report observations, per-item durable Delivery effects, and correction history bound to a confirmed Task and accepted criterion pass synthetic tests. A third correction requires diagnosis with a distinct bounded source observation; labels and descriptions cannot reset that threshold. Native Windows source handling, Delivery receipt integration, live supervision, effective launch and cleanup remain unverified. |
...
| P07 | OFFLINE IN PROGRESS | Focused corrected working-tree tests pass; frozen candidate gates will be recorded in the durable task report. The private release gate requires all candidate-bound rows and never authorizes release itself. Hosted, fresh independent, live and matched evaluation remain `NOT_RUN`. |
