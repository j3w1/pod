...
The six supported model definitions, official guidance, guide profiles and dated Artificial Analysis records live in the v2 `skills/pod/catalog.json`. Maintain them as one bounded snapshot: record source URL and capture date, keep each effort/profile label paired with its index, USD per benchmark task, first-response and total-response values, and preserve missing values as `null`. Check the cited official source and benchmark capture before editing; do not synthesize missing measurements. Keep provider descriptions as attributed provider statements of 20–70 words; Pod advice belongs in the guide. The guide limits `suggested_use` to 24 characters, `best_for`, `use_when` and `trade_off` to 62, `limitations` to 55, and each example with effort to 62. Run `PYTHONPATH=skills python -m pod.catalog --check`, focused catalog/TUI tests, visual snapshots and the repository gates. TUI startup reads the bundled catalog only; it makes no network request. Benchmark rows never prove native access, billing or effective model settings.
