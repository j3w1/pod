...
class TuiPtyTests(unittest.TestCase):
    ...
    def test_filter_and_search_focus_match_highlight_details_and_save(self):
        session=self.open()
        session.send('\x1bOB'*4)
        session.wait_for(lambda s:'GPT-6 Sol' in self.focused(s))
        session.settle()
        session.send('f')
        session.wait_for(lambda s:'Filter Claude' in s.text() and 'Claude Sonnet 5' in self.focused(s))
        session.settle()
        self.assertTrue(self.table_line(session,'Claude Sonnet 5').startswith('▸'))
        session.send(' ')
        session.wait_for(lambda _s:self.load(personal=self.config)['saved']['claude-sonnet-5']=='preferred')
        session.send('f')
        session.wait_for(lambda s:'Filter Codex' in s.text() and 'GPT-6 Sol' in self.focused(s))
        session.settle()
        self.assertTrue(self.table_line(session,'GPT-6 Sol').startswith('▸'))
        session.send(' ')
        session.wait_for(lambda _s:self.load(personal=self.config)['saved']['gpt-6-sol']=='preferred')
        session.send('f')
        session.wait_for(lambda s:'Filter All' in s.text() and 'GPT-6 Sol' in self.focused(s))
        session.send('/')
        session.send('Claude')
        session.wait_for(lambda s:'Search Claude' in s.text() and 'Claude Sonnet 5' in self.focused(s))
        session.settle()
        self.assertTrue(self.table_line(session,'Claude Sonnet 5').startswith('▸'))
        session.send('\n')
        session.send(' ')
        session.wait_for(lambda _s:self.load(personal=self.config)['saved']['claude-sonnet-5']=='disabled')
        session.send('\x1b')
        session.wait_for(lambda s:'GPT-6 Sol' in self.focused(s))
        session.settle()
        self.assertTrue(self.table_line(session,'GPT-6 Sol').startswith('▸'))
    ...
    def test_minimum_size_shows_compact_guidance(self):
        session=self.open(cols=40,rows=12)
        text=session.text()
        self.assertIn('Purpose:',text)
        self.assertIn('Pod example:',text)
        self.assertIn('ID claude-opus-5-5; effort auto',text)
        self.assertIn('context native default; Runtime Unknown',text)
    ...
    def test_runtime_unknown_offline_unsupported_and_slow_probe(self):
        ...
        unidentified=self.open(ORCA_CLI_COMMAND=str(stub))
        unidentified.wait_for('Offline')
        stub.write_text("#!/bin/sh\nprintf '%s\\n' '{\"ok\":true,\"result\":{\"runtime\":{\"capabilities\":[\"orchestration.worker-launch-preferences.v1\"]}},\"_meta\":{\"runtimeId\":123}}'\n")
        malformed=self.open(ORCA_CLI_COMMAND=str(stub))
        malformed.wait_for('Offline')
        stub.write_text("#!/bin/sh\nprintf '%s\\n' '{\"ok\":true,\"result\":{\"runtime\":{\"capabilities\":[]}},\"_meta\":{\"runtimeId\":\"fixture-runtime\"}}'\n")
